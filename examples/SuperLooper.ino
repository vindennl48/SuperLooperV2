/*
 * SuperLooper - 8-Track Parallel Guitar Looper
 */
...
// --- Audio Connections ---
AudioConnection      patchL(i2sIn, 0, looper, 0);
AudioConnection      patchOutL(looper, 0, i2sOut, 0);
AudioConnection      patchOutR(looper, 0, i2sOut, 1); 
...
void setup() {
    ...
    // --- TrackManager Initialization ---
    if (!trackManager.init()) {
        Serial.println("TrackManager Init Failed!");
    }
    looper.setTrackManager(&trackManager);

    LOG("SuperLooper 9-Track Initialized (IDLE)");
}
